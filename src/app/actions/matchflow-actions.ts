 ...
 * All critical financial updates are now delegated to Supabase Edge Functions 
 * where the Service Role key and environment variables are stored.
 */
...
export async function sendGiftAction(senderUid: string, recipientUid: string, coinAmount: number, giftName: string) {
  try {
    ...
    if (error) throw error;
    return data;
  } catch (error: any) { return { success: false, error: "Gift delivery failed via edge function." }; }
}
