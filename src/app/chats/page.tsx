...
function ChatsContent() {
  ...
  const handleClear = async () => {
    ...
    await clearChatAction(currentUser.id, chatToDelete); setChatToDelete(null);
  };
  ...
  if (!startWithId) return (
    <div className="flex-1 bg-white min-h-screen relative select-none">
      ...
      <AlertDialog open={!!chatToDelete} onOpenChange={() => setChatToDelete(null)}><AlertDialogContent className="rounded-[2rem] p-8 max-w-[85vw]"><AlertDialogHeader><AlertDialogTitle className="font-black text-center uppercase tracking-tight">Delete Chat?</AlertDialogTitle></AlertDialogHeader><AlertDialogFooter className="gap-3 mt-4"><AlertDialogCancel className="h-12 rounded-xl font-black text-[10px] uppercase tracking-widest border-none bg-gray-50">Keep</AlertDialogCancel><AlertDialogAction onClick={handleClear} className="h-12 rounded-xl bg-red-500 font-black text-[10px] uppercase tracking-widest border-none text-white shadow-lg shadow-red-100">Delete</AlertDialogAction></AlertDialogFooter></AlertDialogContent></AlertDialog>
    </div>
  );
