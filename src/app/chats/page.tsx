...
function ChatsContent() {
  ...
  const fetchSummaries = useCallback(async () => {
    ...
    const validChats = chatsData.filter(c => {
      ...
      // ONLY SHOW IF LAST MESSAGE IS NEWER THAN CLEAR TIMESTAMP
  ...
  // CHAT DETAIL INITIALIZATION
  useEffect(() => {
    if (currentUser?.id && startWithId) {
      const ids = [currentUser.id, startWithId].sort()
      const cId = `direct_${ids[0]}_${ids[1]}`
      
      // RESET ALL DETAIL STATES IMMEDIATELY
      setMessages([])
      setChatId(cId)
      setPartnerProfile(null)
      setActiveChatClearedAt(-1)
      
      markChatAsReadAction(currentUser.id, cId);
      
      // FETCH PARTNER & CLEAR TIMESTAMP ATOMICALLY
      Promise.all([
  ...
  useEffect(() => {
    ...
    const fetchMessages = async () => {
      const { data } = await supabase
        ...
        .eq('chat_id', chatId)
        .gt('timestamp', activeChatClearedAt) // RIGOROUS FILTERING
    ...
    const channel = supabase.channel(`messages:${chatId}`)
      .on('postgres_changes', { event: 'INSERT', table: 'messages', filter: `chat_id=eq.${chatId}` }, (payload) => {
        ...
        setMessages(prev => {
          // Accurate Deduplication for Optimistic UI
  ...
  const handleSendMessage = useCallback(async (customText?: string) => {
    ...
    if (!res.success) {
      ...
      toast({ variant: "destructive", title: "Error", description: "Could not send." })
    }
  ...
  const handleSendGift = async (gift: typeof GIFTS[0]) => {
    ...
    // PRE-CHECK COINS BEFORE OPTIMISTIC UPDATE
  ...
  if (!startWithId) return (
    <div className="flex-1 bg-white min-h-screen pb-20 select-none animate-in fade-in duration-200">
      ...
      <main className="flex flex-col">
        {chatSummaries.length === 0 ? (
          ...
          <div key={s.id} onPointerDown={() => handleTouchStart(s.id)} onPointerUp={() => handleTouchEnd(s.partner_id)} onPointerMove={handleTouchMove} className="p-5 flex items-center gap-4 active:bg-gray-50 border-b border-gray-50 transition-colors cursor-pointer touch-none">
      ...
      <AlertDialog open={!!deletingChatId} onOpenChange={(open) => !open && setDeletingChatId(null)}>
        <AlertDialogContent className="rounded-[2.5rem] max-w-[85vw] p-8 border-none shadow-2xl">
          <AlertDialogHeader className="items-center text-center"><AlertDialogTitle className="text-xl font-bold">Delete conversation?</AlertDialogTitle><AlertDialogDescription className="text-xs uppercase font-bold tracking-widest text-gray-400">Message history will be cleared.</AlertDialogDescription></AlertDialogHeader>
          <AlertDialogFooter className="flex flex-row items-center justify-center gap-4 mt-6">
            ...
            <AlertDialogAction onClick={() => { if (deletingChatId) handleClearChat(deletingChatId); setDeletingChatId(null); }} className="flex-1 h-14 rounded-full bg-red-500 text-white shadow-lg">Delete</AlertDialogAction>
          </AlertDialogFooter>
        </AlertDialogContent>
      </AlertDialog>
    </div>
  ...
  return (
    <div className="flex flex-col h-screen bg-white select-none overflow-hidden animate-in fade-in duration-200">
        ...
        {messages.map(m => {
          ...
          return (
            <div key={m.id} className={cn("max-w-[80%] p-4 rounded-[2rem] text-sm font-medium shadow-sm relative", 
              ...
              !m.is_optimistic && "animate-in zoom-in-95 duration-200"
            )}>
              {m.is_gift ? (
